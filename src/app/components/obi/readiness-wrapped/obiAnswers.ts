...
const ANSWER_RULES: AnswerRule[] = [
  {
    matches: ({ question }) =>
      question.includes('department') && includesAny(question, ['weak', 'help', 'most']),
    answer: () => {
      const weakest = [...ORG.departments].sort((a, b) => a.score - b.score)[0];
      return `${weakest.name} scores lowest at ${weakest.score} (${weakest.n} people assessed). Finance Systems (${ORG.departments[6].score}) and Business Analysis (${ORG.departments[5].score}) are also below the org average of ${ORG.readinessAvg}. Engineering leads at ${ORG.departments[0].score}.`;
    },
  },
  {
  ...
  },
  {
    ...
    answer: () =>
      `Your org averages ${ORG.readinessAvg} readiness, but Scaling & Enablement sits at 2.6/5 while Mindset is 4.1 — people are willing but can't spread wins. Meanwhile ${dormantLicensePct}% of Copilot licenses (${ORG.copilot.dormant} seats) are idle.`,
  },
  {
    matches: ({ question, context }) => question.includes('scaling') || context.includes('shape read'),
    answer: () =>
      `Scaling & Enablement is the weakest dimension at 2.6/5 org-wide. Engineering (${ORG.departments[0].score}) is the bright spot — their scaling practices separate them from the ${ORG.readinessAvg} average. Mindset (${ORG.dimensions[0].v}) shows willingness isn't the blocker.`,
  },
  {
    matches: ({ question, context }) =>
      includesAny(question, ['distribution', 'spread']) || context.includes('spread'),
    answer: () =>
      `${ORG.levels.Skilled} Skilled, ${ORG.levels.Familiar} Familiar, ${ORG.levels.Learner} Learner, and ${ORG.levels.Beginner} Beginner (${ORG.assessed} assessed). ${learnerOrBelow} people sit at Learner or below while 96 Skilled pull the ${ORG.readinessAvg} average up.`,
  },
  {
    matches: ({ question, context }) => question.includes('engineering') || context.includes('bright spot'),
    answer: () =>
      `Engineering leads at ${ORG.departments[0].score} with ${ORG.departments[0].n} people assessed. Their edge is specifically scaling — the same capability stuck at 2.6 org-wide. ${ORG.copilot.builders} builders (of ${ORG.copilot.agents} agents) concentrate here.`,
  },
  {
    matches: ({ question, context }) =>
  ...
  },
  {
    ...
    answer: () => {
      ...
      return `${ORG.roles.top.length + ORG.roles.tailRoles} distinct roles across ${ORG.assessed} assessed. Top roles: ${top}. Analyst cluster (Business + Data + Cyber) = ${ORG.roles.analystClusterPeople} people. Long tail: ${ORG.roles.tailRoles} roles, ${ORG.roles.tailPeople} people.`;
    },
  },
  {
    matches: ({ question }) => includesAny(question, ['assessment', 'gap', '812']),
    answer: () =>
      `You've assessed ${ORG.assessed} of ${ORG.peopleInScope} people (${coveragePct}% coverage) across ${ORG.departmentsCount} departments. Closing the gap unlocks whole-org momentum tracking on reassessment.`,
  },
  {
...
function actionAnswer(input: AnswerInput): string | null {
  ...
  if (input.context.includes('scaling sprint') || input.question.includes('scaling')) {
    return `Start by pairing ${ORG.copilot.builders} builders with 2–3 teams scoring below ${ORG.readinessAvg} on scaling. Engineering can host a 2-week sprint; expect measurable lift in Scaling & Enablement (currently 2.6). Owner: Engineering lead + AI CoE.`;
  }

  if (input.context.includes('dormant') || input.question.includes('reclaim')) {
    return `Run a 30-day activation campaign targeting ${ORG.copilot.dormant} dormant license holders. Reallocate unresponsive seats after 60 days. Owner: IT Ops + department leads. High impact, low effort — direct budget recovery.`;
  }

  if (input.context.includes('analyst') || input.question.includes('cluster')) {
    return `Design one enablement program for the ${ORG.roles.analystClusterPeople}-person analyst cluster (Business, Data, Cyber analysts). Highest leverage per program dollar. Owner: Data & Analytics or Business Analysis lead.`;
  }
